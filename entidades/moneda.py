...
class Moneda:
    ...
    def golpea(self, x: int, y: int) -> bool:
        """Este método comprueba si un punto golpea a la moneda, es decir está dentro de la hitbox

        @param x: es la posicion x del punto
        @param y: es la posicion y del punto
        @return: True si golpea, False si no"""

        if (self.__x <= x <= self.__x + self.sprite[3]
    ...
    @property
    def eliminar(self):
        """Devuelve si la moneda debe eliminarse"""
